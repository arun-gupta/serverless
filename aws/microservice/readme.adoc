...
. Create bucket: `aws s3 mb s3://microservice-serverless`
. Copy JAR file to S3 bucket: `aws s3 cp microservice-http-endpoint/target/microservice-http-endpoint-1.0-SNAPSHOT.jar s3://microservice-serverless/microservice-http-endpoint-1.0-SNAPSHOT.jar`
. Deploy SAM
+
```
